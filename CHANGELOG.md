...
### 🚀 Highlights
- **Prefix-only spec edits are processed** (#95 — fixes #94; the reason this release is a minor, not a patch): changing `spec.prefixes` on a PermissionBinder while the ConfigMap and `spec.roleMapping` stayed untouched was a no-op, so RoleBindings under the old prefix survived and `status.conditions[Processed].observedGeneration` stayed behind until the next ConfigMap or roleMapping change — the opposite of the README's "Prefix Change → Removes old RoleBindings, creates new ones". The CRD status gains **`lastProcessedGeneration`**, the skip guard now compares the generation as well, and `reconcileAllManagedResources` runs on a spec change, so a prefix change deletes the managed RoleBindings whose Group subject matches none of the current `spec.prefixes`.
- **arm64 image build and the multi-arch e2e test are paused** (#96): the release image is **`linux/amd64` only** — no `v*.*.*-multi-arch` tag trigger, no `multi_arch` workflow_dispatch input, `platforms` always `linux/amd64`, `operator/Makefile` defaults `PLATFORMS` to amd64 and the two `operator/scripts/build-multi-arch*.sh` hardcode it, and e2e test 12 (Multi-Architecture Verification) is out of the default pools. Every change is commented out, not deleted, with the line to restore next to it.
...
- **Docs and repo hygiene** (#86 — closes #85): `SECURITY.md`'s supported-versions table, which still read `1.0.x` / `< 1.0` long after the 1.8 line shipped, was moved to the current line (this release adds the `1.9.x` row on top of it and keeps `1.8.x` supported as the last arm64-carrying line), `docs/RUNBOOK.md`, `docs/SRE.md` and `docs/BACKUP.md` carry current version footers, `make test-e2e` runs the live-cluster suite, and the stale `GIT_HISTORY_REWRITE_NOTICE.md` is folded into the 1.6.5 entry.

### 🔐 Security
- **`domain_server` is validated before dialing** (#87): unknown schemes (`ftp://`, `ldapi://`) and host-less values (`ldap:///`, `ldap://:389`, `ldap:///dc=example,dc=com`) are rejected with an explicit `unsupported scheme … (use ldap:// or ldaps://)` / `domain_server … has no host` before any dial, and counted in `permission_binder_ldap_connections_total{status="error"}`. Without that check the normalized URL would leave go-ldap dialing `:389` on the pod's own loopback; before the fix such values were mangled like every other `ldap://` value and dialed the literal host `ldap`, which inside a cluster is whatever Service happens to be called `ldap`.
- **The bash release gate is linted** (#91): the two e2e runners, `cleanup-operator.sh`, `test-common.sh` and the 62 test bodies are shellcheck-clean at `--severity=warning` in CI — the scripts that gate every release had never been linted. The `OPERATOR_IMAGE` override accepts only `A-Za-z0-9._/:@-` and is validated once up front by the parallel runner (#88).
- No dependency or CVE change since v1.8.1 — `go.mod`/`go.sum` are untouched.

### ⚙️ Behavior Changes (operational, not correctness)
- **A spec-only change is now reconciled** (#95): the guard skips only when the ConfigMap version, the roleMapping hash **and** `metadata.generation` are all unchanged (new log line `ConfigMap, role mapping and spec have not changed, skipping reconciliation`, replacing `ConfigMap and role mapping have not changed, skipping reconciliation`). A prefix change deletes the managed RoleBindings whose Group subject matches none of the new `spec.prefixes` — the README-documented behavior, dead until now. A CR last processed by an older operator (`lastProcessedGeneration` 0 or absent) gets one extra full, idempotent pass after the upgrade.
- **New CRD status field `status.lastProcessedGeneration`** (`int64`, optional) (#95): the `metadata.generation` last processed to completion. Like `lastProcessedConfigMapVersion` since the v1.8.0 status-poison fix (#53), it is never stamped on an incomplete pass, and `status.conditions[Processed].observedGeneration` now advances on every processed pass.
- **No arm64 image is published for this release** (#96). Every published tag carries `linux/amd64` only — the version tags `1.9.0`, `1.9` and `1` and, since #96 landed on `main`, the floating `latest`; an arm64 node pulling any of them fails with `no matching manifest for linux/arm64`. The arm64 binary and image are still buildable on demand (`make build-static-arm64`, or `make docker-buildx PLATFORMS=linux/arm64,linux/amd64`; the `build-multi-arch*.sh` scripts hardcode amd64 and ignore the variable).
- **`ldap://` values start working on upgrade** (#87): a `domain_server` beginning with `ldap://` always failed before and now connects — anything silently broken under that form becomes live. A bare `host[:port]` keeps meaning plain LDAP; `ldaps://` is unchanged apart from the error text. Upper-/mixed-case schemes and whitespace-padded values (the classic `echo x | base64` in YAML) now work instead of failing; unknown schemes (`ftp://`, `ldapi://`) and host-less values (`ldap:///`, `ldap://:389`) are rejected before dialing with an explicit error and counted in `permission_binder_ldap_connections_total{status="error"}`. `ConnectLdap` errors report the URL actually dialed; the `Connected to LDAP server` log line still shows the raw Secret value.

### 🐛 Bug Fixes
- **Prefix-only spec edits no longer skipped** (#95 — fixes #94): changing `spec.prefixes` while the ConfigMap and `spec.roleMapping` stayed untouched was a no-op — the event passed the generation-bump predicate, then the reconciler logged `ConfigMap and role mapping have not changed, skipping reconciliation` and returned, so RoleBindings under the old prefix survived, nothing was logged about the new prefix and `status.conditions[Processed].observedGeneration` stayed at the old generation until the next ConfigMap or roleMapping change. Root cause: the skip guard compared only the ConfigMap `resourceVersion` and the roleMapping hash, so a spec-only change that the generation predicate let through was dropped by the guard one step later. Fix: the status gains `lastProcessedGeneration` (the `metadata.generation` last processed to completion); the guard skips only when ConfigMap version, roleMapping hash AND generation are all unchanged; `reconcileAllManagedResources` now also runs on a spec change, so its existing prefix cleanup (managed RoleBindings whose Group subject matches none of `spec.prefixes`) actually fires; the `Processed` condition's `observedGeneration` advances on every processed pass; and, like the ConfigMap version since the v1.8.0 status-poison fix (#53), the generation is never stamped on an incomplete pass. Two hardening steps from the review: the managed-resource cleanup runs again when the generation moves between the two fetches of one pass (the first cleanup ran against the older spec, and the pass stamps the newer generation), and `createRoleBinding` recreates a RoleBinding whose cached copy vanished server-side before the update (`NotFound` on update → create, the mirror of the existing stale-cache `AlreadyExists` fall-through) — routine now that a prefix change can delete and recreate the same namespace/role RoleBinding in one pass. New envtest regression `prefix_change_regression_test.go` moves a CR from `OLD-PFX` to `NEW-PFX` against an unchanged ConfigMap and asserts the old RoleBinding is deleted, the new one created, the no-change path leaves the status `resourceVersion` untouched, and `lastProcessedGeneration` / `observedGeneration` follow the generation; a new interceptor stale-cache unit test covers the `NotFound`-on-update path; e2e test 02 asserts the prefix-only patch directly (old owned RoleBindings gone, `NEW-PREFIX` RoleBinding created, `lastProcessedConfigMapVersion` unchanged, `available prefixes: [NEW-PREFIX]` in the operator log, restore verified) without the ConfigMap-edit workaround from #93; the `KNOWN GAP (issue #94)` comment is gone.
...
### 🔧 Improvements
- **Example overlays install the generated CRD** (#84 — closes #81): `example/crd/` (a controller-gen v0.17.0 copy without the `networkPolicy` schema) is removed; both `example/environments/*` overlays now reference `example/deployment/crd.yaml`, which `make sync-examples` copies from the generated CRD and the `Tests` workflow keeps in sync via the drift gate; the overlay patch targets the real `operator-controller-manager` Deployment, so the production overlay's `replicas: 2`, image and resource patches finally apply; docs show the `--load-restrictor LoadRestrictionsNone` build.
...
- **shellcheck in CI** (#91 — closes #83): new `shellcheck` job in the `Tests` workflow runs `shellcheck --severity=warning` over every tracked `*.sh` (73 scripts: the two runners, `cleanup-operator.sh`, `test-common.sh`, `generate-large-configmap.sh`, `test-template.sh`, the 62 test bodies, the 3 scripts in `example/rhacs/scripts/` and the 2 in `operator/scripts/`) on every PR and push to `main` — the bash release gate was never linted before. Repo-level `.shellcheckrc` (`shell=bash`, `source-path=SCRIPTDIR` + `SCRIPTDIR/..` with `external-sources=true` so `source "$SCRIPT_DIR/test-common.sh"` resolves from every test, one justified `disable=`: SC2155, 43 of the 54 baseline findings, all in `test-common.sh` and tests 44/45/52, none of which runs under `set -e`, so splitting `local x=$(cmd)` into two lines would be 43 mechanical edits with no behavior change). The 11 remaining findings were fixed without changing behavior: `[[ " ${MISSING_TOOLS[*]} " =~ … ]]` instead of `[@]` (SC2199, the only two errors), `cd "$SCRIPT_DIR" || exit 1` in the per-test loop (SC2164), unused loop counters renamed to `_` (the three `attempt` counters in tests 54/56/61 are not findings under the rc file and were renamed opportunistically), the dead `LOG_ENTRY_FOUND` (test 43) and `METRICS_PORT` (test 56) assignments removed, the unused `pr_num` field of the status `read` dropped and the reserved `expected_content_pattern` parameter of `verify_pr_file_content` marked (SC2034). README gains a "Linting the scripts" section with the local command.
- Unit + envtest (apiserver 1.37.0): **627 pass / 0 fail / 3 skip** locally on the tested commit `eef948f` and green in the `Tests` workflow (`lint`, `shellcheck`, `test` jobs); `golangci-lint` 0 issues, `shellcheck` 0 findings. The +22 over v1.8.1's 605 are `TestNormalizeLdapURL` (1 parent + 19 subtests) and `TestConnectLdapRejectsUnsupportedScheme` from #87, plus `TestCreateRoleBinding_UpdateNotFoundStaleCache` from #95; the envtest Ginkgo suite grew from 1 to 2 specs with #95's `prefix_change_regression_test.go` (Ginkgo specs are not counted individually in the Go tally).
- Isolated 61-test parallel e2e suite on a live k3s v1.35.7 cluster against the tested image `sha-eef948f` (index digest `sha256:3716e1e5309f01bc2ca675553a6d6c6219c0b0d8cf64f69a78deff3a7f764cbd`, one `linux/amd64` child `sha256:7e0bfc7e94ed…` plus the attestation manifest), deployed through the `OPERATOR_IMAGE` override; the validation cluster is mixed-architecture, so the operator Deployment additionally carried a local, uncommitted `nodeSelector: kubernetes.io/arch: amd64` for the run, because the image is amd64-only (#96): **58 pass / 3 fail** in-suite, **59 of 61 effective** after one targeted re-run, wall-clock 3316 s (parallel phase 1331 s, 43/43; pool C serial 1822 s, 15/18) (suite `20260909-130400`). Test 61 (LDAP mock) failed in-suite on its LDAPS sanity check — an in-pod `ldapsearch` over `ldaps://` against the generated CA, before the operator is involved — and passed on a targeted re-run with all 14 assertions including both plain-`ldap://` ones; the test is unchanged since the green run earlier that day, so the in-suite failure was environmental (pool-C load). Test 12 (Multi-Architecture Verification) is no longer in the default pools (#96). Tests 54 and 56 fail on the `pr-merged` assertion (#82, #60); test 49 only info-logs that state and passes its label assertion because the harness attaches the `auto-merge` label itself (#56). Tests with new or first-time-real assertions are part of the tally: 02 (prefix-only patch, #95), 25 (pod-scoped Prometheus poll, #93), 61 (plain-`ldap://` phase, #87) and 16 (real RBAC-loss test, #90).

### 📝 Documentation
- **Docs and repo hygiene refresh** (#86 — closes #85): the `SECURITY.md` supported-versions table replaced the long-stale `1.0.x` / `< 1.0` rows with the current line (`1.9.x` added and `1.8.x` kept in this release), `docs/RUNBOOK.md`, `docs/SRE.md` and `docs/BACKUP.md` carry current version footers, and the stale `GIT_HISTORY_REWRITE_NOTICE.md` is folded into the 1.6.5 entry.
- The skip-guard contract is documented where it is read (#95): README "Key Behaviors" gains a **Spec Change** row, `operator/README.md` and `docs/SEQUENCE_DIAGRAMS.md` describe the three-input guard, `docs/API_REFERENCE.md` documents `status.lastProcessedGeneration` and corrects the `lastProcessedConfigMapVersion` behavior text, and `example/tests/scenarios/02-prefix-changes.md` matches the rewritten test.
...
- `example/README.md` shows the `kubectl kustomize --load-restrictor LoadRestrictionsNone environments/<env>/ | kubectl apply -f -` build the overlays need, and `example/ARGOCD.md` documents the matching `kustomize.buildOptions` setting in `argocd-cm` (instance-wide, or `kustomize.buildOptions.<version>` per registered Kustomize version; repo-server restart required — the Argo CD Application CRD has no per-Application equivalent) (#84).

### 🐞 Known Issues
- **No arm64 image for this release** (#96): the published `1.9.0` tags are `linux/amd64` only, and `latest`, `1` and `1.9` resolve to that same amd64-only image (`latest` since #96 landed on `main`) — only pinned tags up to `1.8.1` still carry arm64, and an arm64 node pulling any of the others fails with `no matching manifest for linux/arm64`. arm64 users must pin `1.8.1` explicitly or build the image locally (`make docker-buildx PLATFORMS=linux/arm64,linux/amd64`; the `build-multi-arch*.sh` scripts hardcode amd64). E2e test 12 (Multi-Architecture Verification) is out of the default pools and only runs when requested by id.
- **GitHub auto-merge labels are silently dropped** (#56, closed as not planned 2026-08-27): the label list is sent in the `POST /pulls` payload, which GitHub ignores. E2e test 49 asserts the label but attaches it itself first (`gh pr edit --add-label auto-merge`), so it passes without the operator ever setting it.
- **GitHub `pr-merged` is never recorded** (#82, open — not scheduled for this release): the GitHub PR decoders compare `state` (`open`/`closed`) against `MERGED`, a GitLab/Bitbucket value, and never read `merged_at`, so even a successful auto-merge leaves the entry `pr-pending` and everything keyed on `pr-merged` (drift detection, template-change detection, the periodic pass) never runs for GitHub-backed repositories. E2e tests 54 and 56 fail on the `pr-merged` assertion; test 49 only info-logs the state. #82 is the prerequisite for #60.
...
### 📦 Upgrade Notes
- **Apply the CRD before the image**: the only API change since v1.8.1 is the new optional status field `status.lastProcessedGeneration` (#95) — additive, no removals, nothing else in the schema moved. Under the old CRD the API server prunes the unknown field, so the operator reads it back as `0` on every pass and never takes the skip path on generation: correct but permanently un-skipped reconciliation. Apply the regenerated CRD (`example/deployment/crd.yaml`, or `operator/config/crd/bases/…`) before rolling the Deployment. No RBAC or dependency change (`go.mod`/`go.sum` untouched since v1.8.1); the only manifest changes since v1.8.1 are the regenerated CRD and the example image tag.
- **Prefix-change semantics** (#95): after the upgrade a `spec.prefixes` edit is acted on — managed RoleBindings whose Group subject matches none of the new prefixes are deleted, which is the documented behavior but was inert before. Review pending prefix edits before rolling out. Every CR gets one extra full, idempotent reconcile pass on first contact with the new operator (`lastProcessedGeneration` starts at 0/absent).
- **arm64 is paused** (#96): `1.9.0` publishes `linux/amd64` only, and so do the floating tags — `1`, `1.9` and `latest` (the last one already since #96 landed on `main`) all resolve to a `linux/amd64`-only image, so an arm64 node pulling any of them fails with `no matching manifest for linux/arm64`. Clusters with arm64 nodes must pin `1.8.1` explicitly (the last multi-arch tag) or build the arm64 image themselves; a cluster that merely tracks `latest` or `1` breaks without any version change on its side. Repo docs still point arm64 readers at floating tags (`docs/RUNBOOK.md` `docker pull …:latest`, `example/README.md` `image: …:latest`) — that cleanup is part of the #96 follow-up. Nothing else in the release depends on the architecture.
- **`ldap://` now works** (#87): review `domain_server` values that begin with `ldap://` — they were silently broken and will connect after the upgrade. Values with an unknown scheme or no host are rejected before dialing with an explicit error (both were errors before, now with a readable message).
- **Example overlays** (#84): build them with `kubectl kustomize --load-restrictor LoadRestrictionsNone environments/<env>/ | kubectl apply -f -`, or set `kustomize.buildOptions: "--load-restrictor LoadRestrictionsNone"` in `argocd-cm` for Argo CD — the overlays reference files above their own directory, so a plain `kubectl apply -k environments/<env>/` has never built with the default load restrictor (this is not new in the 1.8.x/1.9.0 line). What is new is that the build now renders something correct: the generated CRD and, in the production overlay, the `replicas: 2`, image and resource patches, which silently applied to nothing before. Anyone who applied `example/crd/` directly should apply `example/deployment/crd.yaml` instead.
- **E2E harness**: `KUBECONFIG` must be set or `$HOME/.kube/config` present (the implicit `~/workspace01/k3s-cluster/kubeconfig1` path is gone) and `cleanup-operator.sh` refuses to run against an unreachable cluster instead of printing `CLEANUP COMPLETE`. Runner variables `OPERATOR_IMAGE`, `OPERATOR_IMAGE_PULL_POLICY` (default `Always`), `GITHUB_GITOPS_SECRET_FILE`, `GITHUB_GITOPS_READONLY_SECRET_FILE`, `E2E_WAIT_MULT`, `INSTANCE`, `E2E_ALLOW_LEGACY` are documented in `example/tests/README.md`. Legacy-mode cleanup deletes whatever the default `MANAGED_BY_VALUE` manages — test clusters only; new tests must name namespaces `${TEST_NS_PREFIX}test-NN-<slug>` or extend `TEST_NS_ALLOWLIST` (`ADDING_NEW_TESTS.md`).
- Contributors: run `make sync-examples` after changing the CRD (the `Tests` drift gate fails otherwise), keep `shellcheck --severity=warning $(git ls-files '*.sh')` clean (`.shellcheckrc` at the root), and `make test-e2e` now needs a reachable cluster (`E2E_ARGS` passes flags through). `make docker-buildx` builds `$(PLATFORMS)`, now `linux/amd64` (override with `make docker-buildx PLATFORMS=linux/arm64,linux/amd64` — a make-only lever; the `build-multi-arch*.sh` scripts hardcode amd64 and ignore the variable), and `make build-static` builds the amd64 binary only — `make build-static-arm64` still builds the arm64 one (#96).
- OpenShift / policy-based verification: pin `cosign.pub` (`ClusterImagePolicy`, `policyType: PublicKey`) — the key pair does not change between releases. Keyless verification keeps working (identity is tag-specific, `…@refs/tags/v1.9.0`).
...
[Unreleased]: https://github.com/lukasz-bielinski/permission-binder-operator/compare/v1.9.0...HEAD
[1.9.0]: https://github.com/lukasz-bielinski/permission-binder-operator/compare/v1.8.1...v1.9.0
