...
### 🐛 Bug Fixes
- **Prefix-only spec edits no longer skipped** (#95 — fixes #94): changing `spec.prefixes` on a PermissionBinder while the ConfigMap and `spec.roleMapping` stayed untouched was a no-op — the event passed the generation-bump predicate, then the reconciler logged `ConfigMap and role mapping have not changed, skipping reconciliation` and returned, so RoleBindings under the old prefix survived, nothing was logged about the new prefix and `status.conditions[Processed].observedGeneration` stayed at the old generation until the next ConfigMap or roleMapping change — the opposite of the README's "Prefix Change → Removes old RoleBindings, creates new ones". Root cause: the skip guard compared only the ConfigMap `resourceVersion` and the roleMapping hash, so a spec-only change that the generation predicate let through was dropped by the guard one step later. Fix: the status gains `lastProcessedGeneration` (the `metadata.generation` last processed to completion); the guard skips only when ConfigMap version, roleMapping hash AND generation are all unchanged (new log line `ConfigMap, role mapping and spec have not changed, skipping reconciliation`); `reconcileAllManagedResources` now also runs on a spec change, so its existing prefix cleanup (managed RoleBindings whose Group subject matches none of `spec.prefixes`) actually fires; the `Processed` condition's `observedGeneration` advances on every processed pass; and, like the ConfigMap version since the v1.8.0 status-poison fix (#53), the generation is never stamped on an incomplete pass. Behavior note: a prefix change now deletes the RoleBindings whose group matches none of the new prefixes — the README-documented behavior — and a CR last processed by an older operator (`lastProcessedGeneration` 0/absent) gets one extra full, idempotent pass after the upgrade. New envtest regression `prefix_change_regression_test.go` moves a CR from `OLD-PFX` to `NEW-PFX` against an unchanged ConfigMap and asserts the old RoleBinding is deleted, the new one created, the no-change path leaves the status `resourceVersion` untouched, and `lastProcessedGeneration` / `observedGeneration` follow the generation; e2e test 02 asserts the prefix-only patch directly (old owned RoleBindings gone, `NEW-PREFIX` RoleBinding created, `lastProcessedConfigMapVersion` unchanged, `available prefixes: [NEW-PREFIX]` in the operator log, restore verified) without the ConfigMap-edit workaround from #93; the `KNOWN GAP (issue #94)` comment is gone. Two hardening steps from the review: the managed-resource cleanup runs again when the generation moves between the two fetches of one pass (the first cleanup ran against the older spec, and the pass stamps the newer generation), and `createRoleBinding` recreates a RoleBinding whose cached copy vanished server-side before the update (`NotFound` on update → create, the mirror of the existing stale-cache `AlreadyExists` fall-through) — routine now that a prefix change can delete and recreate the same namespace/role RoleBinding in one pass; covered by a new interceptor stale-cache unit test.

### 🧪 Testing & CI
- **arm64 build and multi-arch e2e test disabled (commented out, not removed)**: `docker-build-push.yml` no longer reacts to `v*.*.*-multi-arch` tags or the `multi_arch` dispatch input and builds `linux/amd64` only (QEMU step and the multi-arch branch of the platform selection stay in the file as comments); `operator/Makefile` `PLATFORMS` defaults to `linux/amd64` (override `PLATFORMS=linux/arm64,linux/amd64` to build both) and `build-static` skips the arm64 binary (`build-static-arm64` still builds it on demand); `operator/scripts/build-multi-arch*.sh` default to `linux/amd64`; e2e test 12 (Multi-Architecture Verification) is dropped from `run-tests-parallel.sh` Pool A and from the no-argument test list in `run-tests-full-isolation.sh` (it still runs when requested explicitly by id). Every change carries a comment showing the line to restore.
- **E2E test 25 made timing-robust, test 02 made a real test** (#92): test 25 (Prometheus Metrics Collection) replaced its fixed `sleep 45` + single 30s retry (75s total — shorter than the ~90-120s prometheus-operator config-propagation worst case measured on kube-prometheus-stack 90.0.0, where kubelet Secret propagation alone took ~84s) with an `E2E_WAIT_MULT`-scaled poll of up to 180s in 15s steps, and the query is now scoped to the current operator pod via `pod="<pod name>"` (prometheus-operator stamps the `pod` label on every ServiceMonitor target) so a re-run can no longer "pass" on the previous pod's stale series inside Prometheus' 5-minute lookback window; it fails fast when the pod name is unreadable. Test 02 (Prefix Changes) previously contained no assertions at all (it passed on exit code); it now asserts — with a `fail_test` path per assertion — that the operator processed the new prefix and that a `NEW-PREFIX` ConfigMap entry yields its namespace and an owned RoleBinding, then restores the original `COMPANY-K8S` prefix and drops the new-prefix entry. The assertions run after the ConfigMap edit because a prefix-only spec change is not applied by the operator on its own (skip guard gap, tracked as #94 — closed by #95 above, which removes the workaround again); `scenarios/02-prefix-changes.md` updated to match.
