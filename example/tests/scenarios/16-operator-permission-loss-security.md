...
4. Verify the log line is parseable JSON with `error`, `namespace` and `role` fields
5. Verify graceful degradation: namespace `test-16-rbac-loss` created, RoleBinding `test-16-rbac-loss-admin` withheld, PermissionBinder condition `Processed=False` / `ProcessingIncomplete`
...
7. Verify operator recovers: RoleBinding `test-16-rbac-loss-admin` created, `Processed=True` / `ConfigMapProcessed`, Deployment Available
