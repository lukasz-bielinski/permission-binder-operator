...
# The PermissionBinder CR name (test-hyphenated-roles) is NOT prefixed.
HYPH_NS="${TEST_NS_PREFIX}test-hyphenated"
...
data:
  whitelist.txt: |-
    CN=COMPANY-K8S-${HYPH_NS}-read-only,OU=Kubernetes,OU=Platform,DC=example,DC=com
    CN=COMPANY-K8S-${HYPH_NS}-cluster-admin,OU=Kubernetes,OU=Platform,DC=example,DC=com
    CN=COMPANY-K8S-${HYPH_NS}-engineer,OU=Kubernetes,OU=Platform,DC=example,DC=com
EOF

sleep 15

# Verify RoleBindings created for hyphenated roles
if kubectl get namespace "$HYPH_NS" >/dev/null 2>&1; then
    if kubectl get rolebinding "${HYPH_NS}-read-only" -n "$HYPH_NS" >/dev/null 2>&1; then
        pass_test "read-only RoleBinding created"
    else
        fail_test "read-only RoleBinding missing"
    fi
    
    if kubectl get rolebinding "${HYPH_NS}-cluster-admin" -n "$HYPH_NS" >/dev/null 2>&1; then
    ...
    READ_ONLY_ROLE=$(kubectl get rolebinding "${HYPH_NS}-read-only" -n "$HYPH_NS" -o jsonpath='{.metadata.annotations.permission-binder\.io/role}' 2>/dev/null)
    CLUSTER_ADMIN_ROLE=$(kubectl get rolebinding "${HYPH_NS}-cluster-admin" -n "$HYPH_NS" -o jsonpath='{.metadata.annotations.permission-binder\.io/role}' 2>/dev/null)
    ...
    OBSOLETE_LOGS=$(kubectl logs -n $NAMESPACE deployment/operator-controller-manager --tail=100 2>/dev/null | jq -R -r --arg ns "$HYPH_NS" 'fromjson? | select(.message == "Deleted obsolete RoleBinding") | select(.namespace == $ns) | select(.name | contains("read-only") or contains("cluster-admin")) | .namespace + "/" + .name' 2>/dev/null || echo "")
...
EOF
    ...
    if kubectl get rolebinding "${HYPH_NS}-engineer" -n "$HYPH_NS" >/dev/null 2>&1; then
        pass_test "engineer RoleBinding preserved (role still in mapping)"
    else
        fail_test "engineer RoleBinding incorrectly deleted"
    fi
else
    info_log "$HYPH_NS namespace does not exist, skipping hyphenated roles test"
fi
