...
if [ -z "$PROM_POD" ]; then
    ...
    Q_ORPH=$(printf '%s' "permission_binder_orphaned_resources_total{namespace=\"${NAMESPACE:?}\"}" | jq -sRr @uri)
    # Query orphaned resources metric
    ORPHANED_METRIC=$(kubectl_retry kubectl exec -n monitoring $PROM_POD -- wget -q -O- "http://localhost:9090/api/v1/query?query=${Q_ORPH}" 2>/dev/null | jq -r '.data.result[0].value[1]' 2>/dev/null | tr -d '\n' | grep -E '^[0-9]+$' || echo "0")
