...
# Create ConfigMap with 50+ entries (namespaces prefixed per instance)
WHITELIST_LARGE="${RUN_DIR:-/tmp}/whitelist-large.txt"
kubectl_retry kubectl get configmap permission-config -n $NAMESPACE -o jsonpath='{.data.whitelist\.txt}' > "$WHITELIST_LARGE"
for i in {1..50}; do
    echo "CN=COMPANY-K8S-${TEST_NS_PREFIX}large-project-$i-admin,OU=Test,DC=example,DC=com" >> "$WHITELIST_LARGE"
done
kubectl create configmap permission-config -n $NAMESPACE --from-file=whitelist.txt="$WHITELIST_LARGE" --dry-run=client -o yaml | kubectl apply -f - >/dev/null 2>&1
rm -f "$WHITELIST_LARGE"
...
LARGE_NS_COUNT=$(kubectl_retry kubectl get namespaces -l "$MANAGED_BY_LABEL" --no-headers | wc -l)
